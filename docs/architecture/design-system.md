...
├─ base.css            @layer base { * border-border outline-ring/50; html color-scheme light, scroll-padding-top var(--spacing-header);
...
`base.css` sets `scroll-padding-top: var(--spacing-header)` on `html`, the page's scroll container. When the keyboard moves focus to a control above the viewport, or a link targets an in-page anchor, the browser scrolls it to just below the sticky header instead of under it, so focus is never hidden behind the header (WCAG 2.2 SC 2.4.11) and no element needs a `scroll-mt-*` margin of its own.
...
| `menubar.tsx` | `MenubarTrigger` adds `focus-visible:ring-3 focus-visible:ring-ring/50` to the registry's class list, between `hover:bg-muted` and `aria-expanded:bg-muted`. | The registry trigger carries `outline-hidden` and no focus or highlighted style, so a keyboard user moving along the menu bar sees no focus at all (WCAG 2.2 SC 2.4.7); the ring is the one the other generated controls draw. | The kitchen-sink test "reaches the context menu area and the menu bar with the keyboard" tabs to "File" and presses ArrowRight to "Edit", and asserts that the focused trigger has a `box-shadow` and the other none. |
| `tabs.tsx` | `TabsContent` adds `rounded-lg` and `focus-visible:ring-3 focus-visible:ring-ring/50` to the registry's `flex-1 text-sm outline-none`. | Radix gives the tab panel `tabIndex={0}`, so it is a Tab stop, and the registry's `outline-none` leaves it without any focus indicator; `rounded-lg` gives the ring the corners of the tab list. | The kitchen-sink test "navigation and data display controls" tabs from "Overview" and from "Details" into their panels and asserts that each focused panel has a `box-shadow`. |
