...
| `Erp:Platform:Host:MaxRequestBodyBytes` | long | `1048576` | largest request body Kestrel accepts (1 KiB to 1 GiB); a larger body answers 413 `request.too-large`; seeded unlabelled from `infra/appconfig/defaults.json`; an endpoint that must accept more (file uploads) raises its own limit |
...
| `Erp:Platform:RateLimiting:Enabled` | bool | `true` | switches the global rate limiter on; `false` leaves every request unlimited; read once at startup, like every limit below; `RateLimitingOptions`, seeded unlabelled from `infra/appconfig/defaults.json` |
