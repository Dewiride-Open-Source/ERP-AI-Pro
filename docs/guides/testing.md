...
- Names: `Method_Condition_ExpectedResult` for .NET tests; sentence-style titles for `node --test` files under `scripts/**/tests`. One behaviour per test. No sleeps, no order dependence, no shared mutable state.
