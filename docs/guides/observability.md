...
| `DatabaseTelemetryTests` (SQL Server) | a SQL command produces a client span whose query text carries no parameter value; an EF Core query produces `db.client.operation.duration` and `microsoft.entityframeworkcore.queries` |
...
| `HttpClientResilienceTests` | a GET is retried after a 503 and a POST is not |
