...
namespace Dewiride.Erp.Modules.Platform.Attachments.UnitTests.Files.Application.Queries.OpenAttachmentContent;

public sealed class OpenAttachmentContentHandlerTests
{
    [Fact]
    public async Task HandleAsync_ValidLink_HandsTheIdAndTokenToTheServiceAndReturnsTheDownload()
    {
        ...
        var handler = new OpenAttachmentContentHandler(service);

        var result = await handler.HandleAsync(new OpenAttachmentContentQuery(id, "the-link-token"), TestContext.Current.CancellationToken);
    ...
    {
        var handler = new OpenAttachmentContentHandler(new RecordingAttachmentService { Download = AttachmentErrors.NotFound });

        var result = await handler.HandleAsync(new OpenAttachmentContentQuery(AttachmentId.Create(), "expired-token"), TestContext.Current.CancellationToken);
